...
class MultiClassToBanditReduction(BaseBanditDataset):
    ...
    def __post_init__(self) -> None:
        ...
        #     )
        if not isinstance(self.alpha_b, float) or not (0.0 <= self.alpha_b+self.beta_b < 1.0):
            raise ValueError(
                f"alpha_b+beta_b must be a float in the [0,1) interval, but {self.alpha_b+self.beta_b} is given"
            )
    ...
    def split_train_eval(
        ...
        validation_test_bool: Optional[bool] = False,
    ) -> None:
    ...
    def obtain_batch_bandit_feedback(
        self,
        random_state: Optional[int] = None,
        base_classifier_b: Optional[ClassifierMixin] = None, 
        validation_test_bool: Optional[bool] = False,
    ): # -> BanditFeedback:
        ...
        if validation_test_bool:
            base_clf_b = base_classifier_b
        else:
            # train a base ML classifier
            base_clf_b = clone(self.base_classifier_b)
            # base_clf_b.fit(X=self.X_tr, y=self.y_tr) # earlier
            y_rand = np.random.randint(max(self.y_tr)+1, size=self.y_tr.shape)
            base_clf_b.fit(X=self.X_tr, y=y_rand)

        preds = base_clf_b.predict(self.X_ev).astype(int)
        # construct a behavior policy
        #### uniform
        # pi_b = np.zeros((self.n_rounds_ev, self.n_actions))
        # pi_b[:, :] = 1.0 / self.n_actions
        #### non-uniform
        # pi_b = np.zeros((self.n_rounds_ev, self.n_actions))
        ...
        #### 20 Su+
        # pi_b = np.zeros((self.n_rounds_ev, self.n_actions))
        # alpha_plus_rand = self.alpha_b + self.beta_b 
        # pi_b[:, :] = (1.0 - alpha_plus_rand) / self.n_actions
        # pi_b[np.arange(self.n_rounds_ev), preds] = (
        #     alpha_plus_rand + (1.0 - alpha_plus_rand) / self.n_actions
        # )
        #### policy from prob values in classifier 
        pi_b = base_clf_b.predict_proba(self.X_ev).astype(float)
        pi_b[np.where(pi_b == 0)[0],:] = 1 / self.n_actions # non-zero probabilities for positive weights
        ...
        # print("mutliclass",np.sum(reward))
        # import pdb; pdb.set_trace()
        # Add stochastic rewards here.
        # reward = self.sample_reward_given_expected_reward(expected_reward_, action) in synthetic.py
        if self.stoc_reward:
            randomization = np.random.binomial(1, 0.7, reward.shape)
            reward[np.where(randomization==0)] = 1 - reward[np.where(randomization==0)]
    ...
    def obtain_action_dist_by_eval_policy(
        ...
        base_clf_e: ClassifierMixin, default=None
            Learned Machine learning classifier used to construct a behavior policy.
        """
        ...
        if base_classifier_e is None:
        ...
        elif validation_test_bool:  
            preds = base_classifier_e.predict(self.X_ev).astype(int)
            ...
            # )
            #### policy from prob values in classifier
            pi_e = base_classifier_e.predict_proba(self.X_ev).astype(float)
            pi_e[np.where(pi_e == 0)[0],:] = 1 / self.n_actions # non-zero probabilities for positive weights
            #### uniform policy
            # pi_e = np.zeros((self.n_rounds_ev, self.n_actions))
            # pi_e[:, :] = (1.0) / self.n_actions
            return pi_e[:, :, np.newaxis], None
        else:
        ...
        # )
        # policy from prob values in classifier
        pi_e = base_clf_e.predict_proba(self.X_ev).astype(float)
        pi_e[np.where(pi_e == 0)[0],:] = 1 / self.n_actions # non-zero probabilities for positive weights
        # import pdb; pdb.set_trace()
        # uniform policy
        # pi_e = np.zeros((self.n_rounds_ev, self.n_actions))
        # pi_e[:, :] = (1.0) / self.n_actions
        return pi_e[:, :, np.newaxis], base_clf_e
